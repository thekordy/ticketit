@extends($master)
@section('page', trans('ticketit::admin.agent_create_title'))

@section('content')
    @include('ticketit::shared.admin-header')
    <div class="panel panel-default">
        <div class="panel-heading">
            <h2>{{ trans('ticketit::admin.agent_create_title') }}</h2>
        </div>
        @if ($users->isEmpty())
            <h3 class="text-center">{{ trans('ticketit::admin.agent_create_no_users') }}</h3>
        @else
            {!! Form::open(['route'=> config('ticketit.admin_route').'.agent.store', 'method' => 'POST', 'class' => 'form-horizontal']) !!}
            <div class="panel-body">
                {{ trans('ticketit::admin.agent_create_select_user') }}
            </div>
