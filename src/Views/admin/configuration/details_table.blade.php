    ...
    <table class="table table-striped table-bordered table-hover">
        ...
        @foreach($configs as $configuration)
            <tr>
                ...
                <td><a href="{!! route($admin_route.'.configuration.edit', [$configuration->id]) !!}"
                       title="{{ trans('ticketit::admin.table-edit').' '.$configuration->slug }}"
                       data-toggle="tooltip">{!! $configuration->value !!}</a></td>
                <td class="text-center">{!! $configuration->lang !!}</td>
                <td class="text-center">
                    {!! link_to_route(
                        $admin_route.'.configuration.edit', trans('ticketit::admin.btn-edit'), [$configuration->id],
